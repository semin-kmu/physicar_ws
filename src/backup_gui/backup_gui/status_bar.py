...
셀 하나 = 노드 하나. 좌측부터 상태등 · 이름 · Hz 한 줄이다.

    ● backup_steer_controller  49.7Hz
    ● backup_path_planner      -

Hz 칸의 "-" 는 **아직 잴 표본이 모자라다** 는 뜻이다.
...
# 가장 긴 이름(backup_start_signal_detector)이 7.5 pt 에서 ~170 px 이므로
# 170 + 램프 12 + Hz 38 = 220 이 하한이다. 좁게 잡으면 이름이 잘려
# backup_speed_controller 와 backup_steer_controller 가 같은 글자로 보인다.
CELL_W = 220
CELL_H = 15
PAD = 6
    ...
    def paintEvent(self, _ev):
        ...
        if not self._nodes:
            ...
            p.drawText(
                ...
                "감시 대상 없음 (status.enabled=false 이거나 status.watch 가 비었다)")
            return
        ...
        for i, (name, health, hz) in enumerate(self._nodes):
            ...
            # 상태등이 맨 앞이다. 이름 뒤에 두면 짧은 이름 뒤 여백 때문에
            # 램프가 다음 셀 것처럼 보인다.
            p.setPen(QtCore.Qt.NoPen)
            p.setBrush(LAMP.get(health, COL_ABSENT))
            p.drawEllipse(
                QtCore.QPointF(x0 + LAMP_W * 0.5, y0 + CELL_H * 0.5),
                LAMP_R, LAMP_R)
            ...
            p.drawText(
                QtCore.QRectF(x0 + LAMP_W, y0, nw, CELL_H),
                QtCore.Qt.AlignVCenter | QtCore.Qt.AlignLeft,
            ...
            p.setPen(COL_DIM)
            p.drawText(
                QtCore.QRectF(x0 + LAMP_W + nw, y0, HZ_W, CELL_H),
                QtCore.Qt.AlignVCenter | QtCore.Qt.AlignLeft,
