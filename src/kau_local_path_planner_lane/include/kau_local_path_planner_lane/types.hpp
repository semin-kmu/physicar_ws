...
//
// 원본: KAU_AMET_Test / src/kau_local_path_planner_lane/test/config.py 의
//       PlannerParams, src/kau_local_path_planner_lane/test/planner.py 의
//       PlanResult / 후보 튜플 (offset, curve, cost, reason)
// ====================================================================

#ifndef KAU_LOCAL_PATH_PLANNER_LANE__TYPES_HPP_
#define KAU_LOCAL_PATH_PLANNER_LANE__TYPES_HPP_
...
struct PlannerParams
{
    ...
    // 2026-08-25 (lane-only 재설계, KAU_AMET_Test map-free 세션 반영):
    // global path 를 완전히 배제해 `ref_mode`("global"/"lane"/"fuse")/
    // `w_lane`(융합 가중치)/`lane_gate`(융합 제외 거리) 가 전부 의미를
    // 잃었다 -- backbone 은 이제 항상 `/lane/left`,`/lane/right`,
    // `/lane/center` 만으로 만든다(`lane_backbone.hpp`). 세 필드 모두 제거.
    ...
    // w_path_preview 제거 (2026-08-25): global path 곡률 프리뷰 소스가
    // 없어짐(backbone 이 lane edge 기반이라 l_plan 너머 "미래 곡률"을
    // 알 방법이 없음, KAU_AMET_Test Python 세션과 동일 판단).
...
#endif  // KAU_LOCAL_PATH_PLANNER_LANE__TYPES_HPP_
