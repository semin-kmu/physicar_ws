// ====================================================================
// lane_speed_controller_node.cpp
//
// 속도 제어 -- **차선만 보고** 한다. /lane/center 하나가 유일한 입력이다.
// 앞으로 갈 구간의 곡률로 목표속도를 정하고 PID 로 /speed 를 낸다.
//
//     kappa_win = 전방 window 의 최대 |kappa|      (직선이면 ~0)
//     v_ref     = law(kappa_win) -> [v_min, v_max] -> 가감속 제한
//     /speed    = clamp(v_ref + PID(v_ref - v_meas), 0, v_max)
//
// 알고리즘은 kau_control/src/speed_controller_node.cpp 와 **같다.**
// 다른 점은 경로 소스뿐이다 -- local / global fallback 이 없다. 차선을
// 놓치면 다른 경로로 내려가지 않고 0 을 낸다.
//
// ★ 차선 경로는 앞 80 cm 짜리 토막이라 코너를 미리 못 본다. 곡률 기반
//   가감속은 코너에 들어간 뒤에야 걸리므로, 먼저 상수 속도(v_min == v_max)로
//   조향을 확인하고 그다음에 v_min 을 내려 가감속을 켠다.
//
// 출발 게이트 (start_gate.*)
...
//
//     대기 중에도 곡률 -> 목표속도 -> EMA 는 계속 돌린다 (warm).
//
// 속도 피드백(v_meas)
...
#include "kau_control_lane/lane_tracker.hpp"
#include "kau_control_lane/params.hpp"
#include "kau_control_lane/pid.hpp"
...
class LaneSpeedController : public rclcpp::Node
{
public:
    LaneSpeedController()
    : rclcpp::Node("lane_speed_controller"),
      tracker_(this)
    {
        ...
        declare_parameter<double>("speed.v_min", 0.3);
        declare_parameter<double>("speed.v_max", 0.5);
        declare_parameter<double>("speed.look_min_cm", 20.0);
        declare_parameter<double>("speed.look_max_cm", 80.0);
        ...
        RCLCPP_INFO(
            get_logger(),
            "lane_speed_controller 시작. v=%.2f~%.2f m/s 피드백=%s (%s) "
            "출발게이트=%s",
            sp_.v_min, sp_.v_max,
...
private:
    ...
    void onTimer()
    {
        ...
        // 그 값으로 붙는다.
        //
        // v_ref_ 를 0 으로 잡아 두는 것이 급가속 방지의 전부다. 게이트를
        // rateLimit **뒤**에 두면 v_ref_ 가 대기 중 v_max 까지 차올라 허가
        // 순간 계단으로 튄다 -- 순서를 바꾸면 안 되는 이유다.
        if (gate_required_ && !start_permitted_)
        {
            v_ref_ = 0.0;

            // 게이트로 눌려 있는 동안 오차(v_ref - v_meas)를 적분하면
            // 허가 순간 포화된 적분항이 그대로 실린다.
            pid_.reset();
...
int main(int argc, char ** argv)
{
    ...
    // 설정 오류(lane.pose_source 등)는 노드 생성자가 던진다. 그대로 두면
    // terminate 로 죽어 종료 코드가 134 가 되고, supervisor 의 사망 판정이
    // 오염된다. 이유를 남기고 1 로 끝낸다.
    try
    {
        rclcpp::spin(std::make_shared<kau::control_lane::LaneSpeedController>());
    }
    catch (const std::exception & e)
    {
        RCLCPP_FATAL(
            rclcpp::get_logger("lane_speed_controller"),
            "기동 실패: %s", e.what());
