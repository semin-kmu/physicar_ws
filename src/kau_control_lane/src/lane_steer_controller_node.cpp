// ====================================================================
// lane_steer_controller_node.cpp
//
// 조향 제어 -- **차선만 보고** 한다. /lane/center 하나가 유일한 입력이다.
//
//     Ld  = clamp(k_v * v, ld_min, ld_max)         v 는 /speed 구독
//     tgt = 최근접점에서 Ld 앞의 경로점
//     delta = atan(2 * L * sin(alpha) / Ld)  ->  ±max_steer clamp
//
// 알고리즘은 kau_control/src/steer_controller_node.cpp 와 **같다.**
// 다른 점은 경로 소스뿐이다 -- local / global fallback 이 없다. 차선을
// 놓치면 다른 경로로 내려가지 않고 조향 0 을 낸다.
//
// 속도는 lane_speed_controller 가 따로 낸다. 이 노드는 조향만 책임진다.
...
#include "kau_control_lane/pure_pursuit.hpp"
...
class LaneSteerController : public rclcpp::Node
{
public:
    LaneSteerController()
    : rclcpp::Node("lane_steer_controller"),
      tracker_(this)
    {
        ...
        declare_parameter<double>("controller.ld_min_cm", 25.0);
        declare_parameter<double>("controller.ld_max_cm", 80.0);
        ...
        RCLCPP_INFO(
            get_logger(),
            "lane_steer_controller 시작. Ld=%.0f~%.0f cm k_v=%.2f",
...
int main(int argc, char ** argv)
{
    ...
    try
    {
        rclcpp::spin(std::make_shared<kau::control_lane::LaneSteerController>());
    }
    catch (const std::exception & e)
    {
        RCLCPP_FATAL(
            rclcpp::get_logger("lane_steer_controller"),
            "기동 실패: %s", e.what());
