...
using kau::control_lane::Curve;
using kau::control_lane::TrackState;
...
{
    kau::control_lane::ControllerParams p;   // k_v=0.5, 30~175 cm

    EXPECT_NEAR(kau::control_lane::pure_pursuit::lookaheadDistance(1.0, p),
                50.0, 1e-12);           // 0.5 * 1.0 * 100

    EXPECT_NEAR(kau::control_lane::pure_pursuit::lookaheadDistance(0.1, p),
                p.ld_min, 1e-12);       // 5 cm -> 하한

    EXPECT_NEAR(kau::control_lane::pure_pursuit::lookaheadDistance(5.0, p),
...
{
    kau::control_lane::VehicleParams v;      // wheelbase 18 cm

    // 정면 목표 -> 조향 0
    EXPECT_NEAR(
        kau::control_lane::pure_pursuit::steerCommand(
...
{
    kau::control_lane::VehicleParams v;
...
TEST(SpeedParams, CurvatureToTargetSpeed)
{
    ...
    kau::control_lane::SpeedParams s;
