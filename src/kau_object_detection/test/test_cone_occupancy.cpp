...
using kau_object_detection::ConeOccupancyOptions;
using kau_object_detection::ConeOccupancyRejection;
  ...
  const bool wraps_scan_boundary = false)
{
  ...
  return kau_object_detection::compute_cluster_geometry(cluster);
}
...
TEST(ConeOccupancy, ProvisionalCenterIsPushedAwayFromTheSensor)
{
  ...
  const auto candidate =
    kau_object_detection::compute_cone_occupancy_candidate(geometry, options);
...
TEST(ConeOccupancy, WideWallClusterIsRejectedByTheWidthGate)
{
  ...
  ASSERT_TRUE(geometry.valid);
  ASSERT_GT(kau_object_detection::observed_extent_m(geometry), options.maximum_observed_width_m);
...
TEST(ConeOccupancy, ClusterAtTheGateBoundaryIsAccepted)
{
  ...
  ASSERT_TRUE(geometry.valid);
  ASSERT_NEAR(kau_object_detection::observed_extent_m(geometry), 0.30, 1e-12);

  EXPECT_TRUE(
    kau_object_detection::compute_cone_occupancy_candidate(geometry, options).valid);
}
...
TEST(ConeOccupancy, InvalidGeometryIsRejected)
{
  ...
  const auto candidate =
    kau_object_detection::compute_cone_occupancy_candidate(empty_geometry);
...
TEST(ConeOccupancy, GeometryWithoutPointsIsRejected)
{
  ...
  const auto candidate =
    kau_object_detection::compute_cone_occupancy_candidate(geometry);
...
TEST(ConeOccupancy, UnusableOptionsAreRejected)
{
  ...
  EXPECT_EQ(
    kau_object_detection::compute_cone_occupancy_candidate(geometry, zero_radius).rejection,
    ConeOccupancyRejection::kInvalidOptions);

  ConeOccupancyOptions negative_slice;
  negative_slice.visible_slice_radius_m = -0.01;
  EXPECT_EQ(
    kau_object_detection::compute_cone_occupancy_candidate(geometry, negative_slice).rejection,
    ConeOccupancyRejection::kInvalidOptions);

  ConeOccupancyOptions zero_gate;
  zero_gate.maximum_observed_width_m = 0.0;
  EXPECT_EQ(
    kau_object_detection::compute_cone_occupancy_candidate(geometry, zero_gate).rejection,
    ConeOccupancyRejection::kInvalidOptions);

  ConeOccupancyOptions non_finite_gate;
  non_finite_gate.maximum_observed_width_m = std::numeric_limits<double>::quiet_NaN();
  EXPECT_EQ(
    kau_object_detection::compute_cone_occupancy_candidate(geometry, non_finite_gate).rejection,
    ConeOccupancyRejection::kInvalidOptions);
}
...
TEST(ConeOccupancy, BatchApiKeepsOneEntryPerGeometry)
{
  ...
  const auto candidates =
    kau_object_detection::compute_all_cone_occupancy_candidates(geometries, options);
...
TEST(ConeOccupancy, EndToEndTwoBeamConeFromTheClustererYieldsACandidate)
{
  ...
  const auto candidates_clusters =
    kau_object_detection::select_candidate_clusters(clusters);
  ASSERT_EQ(candidates_clusters.size(), 1U);

  const auto geometries =
    kau_object_detection::compute_all_cluster_geometry(candidates_clusters);
  const auto circles =
    kau_object_detection::compute_all_cone_occupancy_candidates(geometries);
