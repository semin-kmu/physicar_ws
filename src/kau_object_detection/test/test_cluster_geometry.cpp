...
using kau_object_detection::Cluster2D;
using kau_object_detection::ClusterGeometry2D;
using kau_object_detection::ClusterGeometryOptions;
using kau_object_detection::ScanPoint2D;
...
{
  const auto geometry = kau_object_detection::compute_cluster_geometry(make_cluster({}));
...
TEST(ClusterGeometry, SinglePointClusterHasZeroWidthAndOneVertex)
{
  ...
  const auto geometry = kau_object_detection::compute_cluster_geometry(cluster);
...
TEST(ClusterGeometry, ShapeMeasuresAreInvariantUnderRotation)
{
  ...
  const auto original = kau_object_detection::compute_cluster_geometry(make_cluster(points));
  const auto rotated =
    kau_object_detection::compute_cluster_geometry(make_cluster(rotated_points));
...
TEST(ClusterGeometry, ShapeMeasuresAreInvariantUnderTranslation)
{
  ...
  const auto original = kau_object_detection::compute_cluster_geometry(make_cluster(points));
  const auto shifted =
    kau_object_detection::compute_cluster_geometry(make_cluster(shifted_points));
...
TEST(ClusterGeometry, NegativeOrNonFiniteToleranceMakesGeometryInvalid)
{
  ...
  EXPECT_FALSE(
    kau_object_detection::compute_cluster_geometry(cluster, negative_options).valid);

  ClusterGeometryOptions non_finite_options;
  non_finite_options.collinear_tolerance_m = std::numeric_limits<double>::infinity();
  EXPECT_FALSE(
    kau_object_detection::compute_cluster_geometry(cluster, non_finite_options).valid);
}
...
TEST(ClusterGeometry, NearlyCollinearPointIsDroppedWithinTolerance)
{
  ...
  const auto coarse =
    kau_object_detection::compute_cluster_geometry(make_cluster(points), coarse_options);
  EXPECT_EQ(coarse.footprint_vertices.size(), 2U);

  ClusterGeometryOptions fine_options;
  fine_options.collinear_tolerance_m = 0.0;
  const auto fine =
    kau_object_detection::compute_cluster_geometry(make_cluster(points), fine_options);
...
TEST(ClusterGeometry, DenseArcClusterStaysConsistent)
{
  ...
  const auto geometry =
    kau_object_detection::compute_cluster_geometry(cluster, exact_options);
...
TEST(ClusterGeometry, ToleranceDoesNotSimplifyAThickCluster)
{
  ...
  const auto exact = kau_object_detection::compute_cluster_geometry(cluster, exact_options);
  const auto tolerant =
    kau_object_detection::compute_cluster_geometry(cluster, tolerant_options);
...
TEST(ClusterGeometry, MatchesClustersProducedByTheClusterer)
{
  ...
  kau_object_detection::ScanClusteringOptions clustering_options;
  ...
  const auto geometries = kau_object_detection::compute_all_cluster_geometry(clusters);
...
{
  EXPECT_TRUE(kau_object_detection::compute_observed_footprint({}, 0.001).empty());
  EXPECT_TRUE(
    kau_object_detection::compute_observed_footprint({make_point(0U, 1.0, 0.0)}, -1.0).empty());
}
