...
kau_object_detection::ScanClusteringOptions test_options()
{
  kau_object_detection::ScanClusteringOptions options;
...
kau_object_detection::ClusterCandidateOptions candidate_options(
  const std::size_t minimum_cluster_points)
{
  kau_object_detection::ClusterCandidateOptions options;
...
kau_object_detection::NeighborFilterOptions disabled_filter()
{
  kau_object_detection::NeighborFilterOptions options;
...
TEST(LaserScanClusterer, ConvertsPolarScanValuesAndPreservesIndices)
{
  ...
  const auto points =
    kau_object_detection::convert_scan_indices_to_points(scan, {0U, 1U});
...
TEST(LaserScanClusterer, SplitsClustersAtLargeGeometricGap)
{
  ...
  const auto clusters = kau_object_detection::cluster_laser_scan(
...
TEST(LaserScanClusterer, RemovesClustersSmallerThanConfiguredMinimum)
{
  ...
  const auto clusters =
    kau_object_detection::cluster_laser_scan(scan, {0U, 1U}, test_options());
  ...
  EXPECT_TRUE(
    kau_object_detection::select_candidate_clusters(clusters, candidate_options(3U)).empty());
  EXPECT_EQ(
    kau_object_detection::select_candidate_clusters(clusters, candidate_options(2U)).size(), 1U);
}
...
TEST(LaserScanClusterer, AdaptiveNeighborDistanceMatchesClusteringLinkThreshold)
{
  ...
  const double threshold = kau_object_detection::adaptive_neighbor_distance_m(
  ...
  EXPECT_GT(
    kau_object_detection::adaptive_neighbor_distance_m(10.0, 10.0, angle_increment, options),
    kau_object_detection::adaptive_neighbor_distance_m(1.0, 1.0, angle_increment, options));

  const double nan_value = std::numeric_limits<double>::quiet_NaN();
  EXPECT_LT(
    kau_object_detection::adaptive_neighbor_distance_m(nan_value, 1.0, angle_increment, options),
    0.0);
}
...
TEST(LaserScanClusterer, ScanIndexFollowsWrapsOnlyForFullCircleScans)
{
  ...
  const auto full_circle_topology =
    kau_object_detection::make_scan_neighbor_topology(full_circle_scan, 0.05);
  ...
  EXPECT_TRUE(kau_object_detection::scan_index_follows(5U, 6U, full_circle_topology));
  EXPECT_TRUE(
    kau_object_detection::scan_index_follows(
      kFullCircleBeamCount - 1U, 0U, full_circle_topology));
  EXPECT_FALSE(
    kau_object_detection::scan_index_follows(
      0U, kFullCircleBeamCount - 1U, full_circle_topology));
  EXPECT_FALSE(kau_object_detection::scan_index_follows(6U, 5U, full_circle_topology));
  EXPECT_FALSE(
    kau_object_detection::scan_index_follows(
  ...
  const auto partial_topology =
    kau_object_detection::make_scan_neighbor_topology(partial_scan, 0.05);

  ASSERT_TRUE(partial_topology.valid);
  EXPECT_FALSE(partial_topology.wraps_full_circle);
  EXPECT_FALSE(kau_object_detection::scan_index_follows(3U, 0U, partial_topology));

  sensor_msgs::msg::LaserScan empty_scan = partial_scan;
  empty_scan.ranges.clear();
  EXPECT_FALSE(
    kau_object_detection::make_scan_neighbor_topology(empty_scan, 0.05).valid);
}
...
TEST(LaserScanClusterer, SpeckleFilterRemovesIsolatedSingletonBeam)
{
  ...
  const auto topology =
    kau_object_detection::make_scan_neighbor_topology(scan, 0.05);
  const auto points =
    kau_object_detection::convert_scan_indices_to_points(scan, usable_indices);
  const auto filtered = kau_object_detection::filter_isolated_scan_points(
  ...
  const auto clusters =
    kau_object_detection::cluster_laser_scan(scan, usable_indices, test_options());
  ASSERT_EQ(clusters.size(), 1U);
...
TEST(LaserScanClusterer, DisabledSpeckleFilterKeepsIsolatedSingletonBeam)
{
  ...
  EXPECT_TRUE(
    kau_object_detection::cluster_laser_scan(scan, {300U}, test_options()).empty());

  const auto kept = kau_object_detection::cluster_laser_scan(
...
TEST(LaserScanClusterer, KeepsAdjacentTwoPointClusterFromDistantObstacle)
{
  ...
  const auto clusters =
    kau_object_detection::cluster_laser_scan(scan, {400U, 401U}, test_options());
...
TEST(LaserScanClusterer, NodeDefaultOptionsAcceptSupportedTwoPointConeCandidate)
{
  ...
  const auto clusters = kau_object_detection::cluster_laser_scan(scan, {400U, 401U});
  ASSERT_EQ(clusters.size(), 1U);
  EXPECT_EQ(clusters.front().points.size(), 2U);

  const auto candidates = kau_object_detection::select_candidate_clusters(clusters);
...
TEST(LaserScanClusterer, KeepsTwoPointClusterAcrossFullCircleBoundary)
{
  ...
  const auto candidates =
    kau_object_detection::select_candidate_clusters(clusters, candidate_options(2U));
...
TEST(LaserScanClusterer, PartialScanDoesNotSupportAcrossFirstAndLastIndex)
{
  ...
  EXPECT_TRUE(
    kau_object_detection::cluster_laser_scan(scan, usable_indices, test_options()).empty());
}
...
TEST(LaserScanClusterer, RequiringTwoSupportingNeighborsDropsTwoPointCandidate)
{
  ...
  kau_object_detection::NeighborFilterOptions strict_filter;
  strict_filter.minimum_support_neighbors = 2U;

  EXPECT_TRUE(
    kau_object_detection::cluster_laser_scan(
...
TEST(LaserScanClusterer, SpeckleFilterKeepsEveryPointThatHoldsAClusteringLink)
{
  ...
  const auto unfiltered = kau_object_detection::cluster_laser_scan(
...
{
  const kau_object_detection::Cluster2D single{{kau_object_detection::ScanPoint2D{}}, false};
  const kau_object_detection::Cluster2D pair{
    {kau_object_detection::ScanPoint2D{}, kau_object_detection::ScanPoint2D{}}, false};
  const std::vector<kau_object_detection::Cluster2D> clusters{single, pair};

  EXPECT_EQ(kau_object_detection::select_candidate_clusters(clusters).size(), 2U);
  EXPECT_EQ(
    kau_object_detection::select_candidate_clusters(clusters, candidate_options(0U)).size(), 2U);
  EXPECT_EQ(
    kau_object_detection::select_candidate_clusters(clusters, candidate_options(2U)).size(), 1U);
  EXPECT_TRUE(
    kau_object_detection::select_candidate_clusters(clusters, candidate_options(3U)).empty());
